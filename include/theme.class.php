...
class Theme {
    ...
    public static function AddJs($sFile, $aggregate = true) {
	if (!$aggregate)
	    $GLOBALS['module_js_not_aggregate'][] = $sFile;
	...
	global $theme, $theme_info, $web_root, $module_js, $compress, $module_js_not_aggregate;
	$module_js_not_aggregate = is_array($module_js_not_aggregate) ? $module_js_not_aggregate : array();
	$module_js = array_unique($module_js);
	$not_packed = array();
	$out_file_path = STATIC_DIR . DS . 'scripts' . DS;
	$staticFileName = implode("", $module_js) . implode("", $theme_info['js']);
	$staticFileName = $out_file_path . 'script-' . md5($staticFileName) . '.js';
	foreach ($module_js as $sFile) {
	    if (array_search($sFile, $module_js_not_aggregate) !== false)
		$not_packed[] = $sFile;
	}
	if (!File::Exists($staticFileName)) {
	    foreach ($module_js as $sFile) {
	    ...
	    if ($theme_info['js'])
		foreach ($theme_info['js'] as $sFile)
		    $out .= "\n\n" . file_get_contents(Theme::GetThemePath($theme) . DS . $sFile) . "\n\n";
	    //$out_file = STATIC_DIR . DS . 'scripts.js';

	    //$compressed = Theme::Compress($out);
	    File::CreateDir(File::FileDir($staticFileName));
	    file_put_contents($staticFileName, $out);
	}
	$scripts = '';
	foreach ($not_packed as $sFile) {
	    if (!preg_match('/^http(.*)/', $sFile))
		$sFile = $web_root . $sFile;
	    $scripts .= '<script type="text/javascript" src="' . $sFile . '"></script>';
	}
	return '<script type="text/javascript" src="' . $web_root . $staticFileName . '"></script>' . $scripts;
    }
